...
# Configuration
INVENTORY_URL="https://inventory-service-production-fff7.up.railway.app"
...
echo "Testing: GET ${INVENTORY_URL}/products-with-stock"
PRODUCTS=$(curl -s "${INVENTORY_URL}/products-with-stock")
echo "Available products:"
echo "$PRODUCTS" | jq '.[0:3]' 2>/dev/null || echo "$PRODUCTS"
echo ""

echo -e "${YELLOW}Step 3: Check Stock for a Specific Product${NC}"
echo "Enter a product ID to test (or press Enter to use LAPTOP001):"
read -r PRODUCT_ID
PRODUCT_ID=${PRODUCT_ID:-LAPTOP001}

echo "Testing: GET ${INVENTORY_URL}/inventory/${PRODUCT_ID}/check/5"
STOCK_CHECK=$(curl -s "${INVENTORY_URL}/inventory/${PRODUCT_ID}/check/5")
